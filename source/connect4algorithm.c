...
/* this file contains the logic that calculates the optimal move for the
 * computer to make in the current gamestate. it is accessed by the main
 * application by way of the fn_CALCULATING_MOVE function: this function signals
 * the move it has decided to make by storing it into the global Move_t
 * move_to_make
 */
        ...
        for (i = 1; i <= 3; i++) {
            ...
            if ((c >= 0) & (c < NUM_COLS) & (r >= 0) & (r < NUM_ROWS)) {
                ...
                if (cells >= 3) {
                    ...
                    if (cells >= 4) {
                        int8_t c_del =
                            c - (dx << 2); // multiply by 4 using shift
                        int8_t r_del = r - (dy << 2);
...
Result_t minimax(Board_t *b, int depth, bool maximizing, Score_t alpha,
                 Score_t beta) {
    ...
    for (c = 0; c < NUM_COLS; c++) {
        ...
        if ((delta == SCORE_MAX) & maximizing) {
            ...
            // recurse
            r = minimax(b, depth - 1, !maximizing, alpha, beta);
...
void fn_CALCULATING_MOVE(void) {
    ...
    //    Display_write();
    //
    //    Result_t r = minimax(&game_board, MAX_DEPTH, true, SCORE_MIN,
    //    SCORE_MAX); Col_t bot_move = r.move; move_to_make = bot_move;
