...
// global variable used only in this file that stores in what column a move from
...
// function that processes an interrupt triggered by a sensor, it gets called by
// the ISRs that pass it the port the interrupt came from
...
void fn_WAITING_FOR_MOVE(void) {
    ...
    // when we get out of sleep it means there's been an interrupt, so let's
    // apply the move we detected
...
void Sensors_init(void) {
    ...
    for (i = 0; i < NUM_COLS; i++) {
        // the sensors we use (tcrt5000) output low when they detect something,
        // so we set them to have pullup resistors and to trigger an interrupt
        // when the input gets low
        GPIO_setAsInputPinWithPullUpResistor(sensors[i].port, sensors[i].pin);
        GPIO_interruptEdgeSelect(sensors[i].port, sensors[i].pin,
                                 GPIO_HIGH_TO_LOW_TRANSITION);
    }

    // enable the interrupts of the gpio ports, the most convenient way to do
    // this is by hardcoding the interrupt numbers
    Interrupt_enableInterrupt(INT_PORT2);
    Interrupt_enableInterrupt(INT_PORT3);
    Interrupt_enableInterrupt(INT_PORT4);
    Interrupt_enableInterrupt(INT_PORT5);
    Interrupt_enableInterrupt(INT_PORT6);
    Interrupt_enableMaster(); // just in case interrupts arent enabled we enable
                              // them
}
...
void Sensors_request_piece(Col_t column) {
    ...
    int i;
    for (i = 0; i < NUM_COLS;
         i++) { // find the sensor that corresponds to the requested column
    ...
    // when we exit from sleep it mean the sensor detected a move on the
    // requested column
    return;
}
...
/*
 * we should detect exactly one move for every time fn_WAITING_FOR_MOVE is
 * called, so all the interrupt routines need mutual exclusion to the
 * move_detected variable and to the Board_t variable to do this we make each
 * interrupt routine disable all the sensor interrupts using
 * disable_interrupts(), so that only one routine can happen at a time
 */
void process_interrupt(uint_fast8_t port) {
    ...
    for (i = 0; i < NUM_COLS; i++) {
        if (sensors[i].port ==
            port) { // only consider sensors belonging to this port
            if (status & sensors[i].pin) { // check if that sensor's pin is the
                                           // one that triggered
                move_detected =
                    sensors[i].column; // set the value of the move we detected
                break;
            }
        }
    }
...
// following here are the implementations of the interrupt routines triggered by
// the sensors each of them just calls the interrupt processing function passing
// it the port it came from

void PORT2_IRQHandler(void) { process_interrupt(GPIO_PORT_P2); }

void PORT3_IRQHandler(void) { process_interrupt(GPIO_PORT_P3); }

void PORT4_IRQHandler(void) { process_interrupt(GPIO_PORT_P4); }

void PORT5_IRQHandler(void) { process_interrupt(GPIO_PORT_P5); }

void PORT6_IRQHandler(void) { process_interrupt(GPIO_PORT_P6); }
